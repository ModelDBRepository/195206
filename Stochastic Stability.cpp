...
#define T 1024*1
#define K 0 //discretization for spectral calculations
...
for (int q=0;q<Q;q++)
{
	...
	D[q] =0.01;//+ D_max*q/((double)Q-1);
	I_o[q]=0.;
	freq[q]=100;
	double omega = freq[q];//(2*Pi)*freq[q]*Dt/dt;
	cout<<q<<"	"<<D[q]<<"	"<<Pi*I_o[q]*I_o[q]/omega/*I_o[q]*I_o[q]*Pi/(((omega*omega)+1)*omega)*/<<endl;	
...
for (int p=0;p<P;p++)
{
			...
			//	cout<<"var nu:"<<var_nu<<endl;
				...
				for (int t=tau;t<T;t++)
				{
						...
						for (int i=0;i<N;i++)
						{
							...
							u[i][t+1]= u[i][t]+dt*(-u[i][t]+F[i][t-delay]+PERIODIC[i][t])+sqrt(2*D[q]*dt)*XI[i][t];
							v[i][t+1]= v[i][t]+dt*(-v[i][t]+F2[i][t-delay]);
							u_spike[i][t+1]=u_spike[i][t]+dt*(-u_spike[i][t]+F3[i][t-delay]+I_o[q]*PERIODIC[i][t])+sqrt(2*D[q]*dt)*XI[i][t];
							...
							V[t+1] = V[t]+dt*(-1.0*V[t]-0.9*1/(1+exp(-5*(V[t-130]-0.25)))+0.9);
						u_ave_spike[t]=0;
						for (int i=0;i<N;i++)
						{
						...
						for (int s=0;s<T;s++)
						{
							...
							A = A + 1/((double) T)*f(u_ave[t-delay]+nu[s]);
						...
						u_ave[t+1]= u_ave[t]+dt*(-u_ave[t]+mean_w*A);
					
						u_mean[t+1]= u_mean[t]+dt*(-u_mean[t]+mean_w*(0.5+(0.5)*erf((0.5)*sqrt(2)*u_mean[t-delay]/sqrt(D[q]))));
				...
				double G=0;
					...
					{
										 double c = c_min+dc*k; 
										 C = C + dc*mean_w*f_prime(u_fixed_point[T-10]+c)*1/sqrt(2*Pi*D[q])*exp(-c*c/(2*D[q]));
									     E = E + dc*mean_w*f_2prime(u_fixed_point[T-10]+c)*1/sqrt(2*Pi*D[q])*exp(-c*c/(2*D[q]));
										 F = F + dc*mean_w*f_3prime(u_fixed_point[T-10]+c)*1/sqrt(2*Pi*D[q])*exp(-c*c/(2*D[q]));
										 //G = G + dc*f(0.25+c)*1/sqrt(2*Pi*D[q])*exp(-c*c/(2*D[q]));
				...
				Fixed_point_theo[q]=-sqrt(Pi)*sqrt(D[q])*mean_w/(sqrt(2)*g-2*sqrt(Pi)*sqrt(D[q]));
				//	Susceptibility[q]=Susceptibility[q]+1/((double)P)*R;
					Susceptibility[q]=Susceptibility[q]+1/((double)P)*(mean_w/sqrt(2*Pi*D[q])*exp(-Fixed_point[q]*Fixed_point[q]/(2*D[q])));
				Susceptibility_theo[q]=mean_w/sqrt(2*Pi*D[q])*exp(-Fixed_point_theo[q]*Fixed_point_theo[q]/(2*D[q]));
				cout<<"R:"<<Susceptibility[q]<<"		R_theo:"<<Susceptibility_theo[q]<<endl;
					...
					for (int t=tau;t<T;t++)
				{
			    	...
			    	//Compute mean EEG
							...
							for (int t=0;t<(int)T;t++)
							{
														...
														mean_3  = mean_3+1/((double) T)*u_ave[t];
	...
	for (int k1=0;k1<100;k1++)
	{
		...
		for (int k2=0;k2<Nc;k2++)
		{
							 ...
							 H = H + dc*f(U[k1]+c)*1/sqrt(2*Pi*Noise_Level)*exp(-c*c/(2*Noise_Level));
    ...
    outfile.close(); 
         ...
         outfile.open("SS  -  test.txt", ios::out);
    for(int k=0;k<(int) T;k++)
    {
    ...
    outfile.close(); 
     ...
     for(int i=0;i<T;i++)
     {
                           outfile<<i*Dt<<"	"<<u[(int)N/2][i]<<"	"<<v[(int)N/2][i]<<"	"<<u_ave[i]<<"	"<<u_mean[i]<<"	"<<u_ave_spike[i]<<"	"<<I_o[0]*PERIODIC[(int)N/2][i]<<endl;
     ...
     for(int q=0;q<Q;q++)
     {
                           outfile<<D[q]<<"	"<<Peak_frequency_1[q]<<"	"<<Peak_frequency_2[q]<<"	"<<Peak_frequency_3[q]<<"	"<<Peak_frequency_4[q]<<"	"<<Theoretical_frequency[q]<<endl;
     ...
     for(int q=0;q<Q;q++)
     {
                           outfile<<D[q]<<"	"<<Susceptibility[q]<<"	"<<Susceptibility_theo[q]<<endl;
